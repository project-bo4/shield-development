	...
	void bdObjectStore::uploadUserObjectsVectorized(service_server* server, byte_buffer* buffer) const
	{
		...
		for (rapidjson::SizeType i = 0; i < objects.Size(); i++) // Uses SizeType instead of size_t
		{
			...
			if (!utils::io::write_file(path, data))
				logger::write(logger::LOG_TYPE_DEBUG, "[bdObjectStore::uploadUserObjectsVectorized] ERROR ON WRITING DATA");
			else
				logger::write(logger::LOG_TYPE_DEBUG, "[bdObjectStore::uploadUserObjectsVectorized] saved %s", name.GetString());
		}
